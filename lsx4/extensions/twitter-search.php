...
class LSXTwitter {
    ...
    public function __construct($type='',$force=false) 
    {
        $this->type = $type;
        $this->force = $force;
        $this->cache_file = TEMPLATEPATH.'/extensions/cache/twitter-search'.$type.'.txt';
    }
    ...
    {        
        ...
        if (file_exists($file))
        {
            ...
            if ($d < 3 && !$this->force)
            {
                echo '<p class="cache-message">from the cache</p>';
                return array('cache',unserialize(trim(file_get_contents($file))));
            } else {
                echo '<p class="cache-message">brand new</p>';
